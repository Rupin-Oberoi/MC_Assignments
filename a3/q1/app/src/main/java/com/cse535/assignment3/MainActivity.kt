        ...
        val sensorEventListener = object : SensorEventListener {
            override fun onSensorChanged(event: SensorEvent) {
                ...
                activity.lifecycleScope.launch(Dispatchers.Main) {
                    ...
                    prevTime = currTime
                    storeData(activity, currTime, x.value, y.value, z.value)
                }
            }
...
private suspend fun storeData(activity: MainActivity , timestamp: Long, x: Float, y: Float, z: Float) {
    val db = AppDatabase.getDatabase(activity)
    val accelerometerData = AccelerometerData(time = timestamp, x = x, y = y, z = z)
...
private suspend fun storeDataCSV(activity: MainActivity){
    ...
    Log.d("csv", csv)
    val file = java.io.File(folderPath, "accelerometer_data.csv")
